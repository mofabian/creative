...
Intelligent_Cops = false;       
%Unimplemented
Intelligent_Agents = false;     
%Unimplemented
Intelligent_Policies = false;   
%Unimplemented, only works if intelligent Cops is enabled
...
%Only makes sense with intelligent people!
ID = 435;
Importance = 10;
...
Each_Round_Legitimacy_Change = -0.01;
    ...
    for i=2:1:length(Prison)
        ...
        if strcmp(A.Get_Type(), 'Agent')
            ...
            if A.Jailtime() <= 0
                ...
                Release = EmptyFields( round(random('Uniform',1,length(EmptyFields),1,1)) );
                ...
                if (strcmp(A.Get_Type(), 'Agent') || strcmp(A.Get_Type(), 'Cop')) && A.Moveable()
                    ...
                    if strcmp(A.Get_Type(), 'Cop') && length(AgentFields) >= 1
                        ...
                        if random('Uniform',0,1,1,1) < Agent_Death_Chance
                            ...
                            Arrest = AgentFields( round(random('Uniform',1,length(AgentFields),1,1)) );
                    ...
                    elseif length(CopFields) >= 1 && A.Grievance(L) - A.Net_Risk(A.Chance_of_Arrest(k, length(CopFields), length(AgentFields)), Jailterm_Max, Better_Deterrent_of_Jailtime) > Murder_threshold && random('Uniform',0,1,1,1) < Murder_success_rate
                        ...
                        GoTo = CopFields( round(random('Uniform',1,length(CopFields),1,1)) );
                    ...
                    elseif length(EmptyFields) >= 1
                        %Wander aimlessly
                        GoTo = EmptyFields( round(random('Uniform',1,length(EmptyFields),1,1)) );
...
%{
x=0;
for i=1:length(AllReleases)
    x = x+AllReleases(i);
end
x / length(AllReleases)
x=0;
for i=1:length(AllCopMoves)
    x = x+AllCopMoves(i);
end
x / length(AllCopMoves)
x=0;
for i=1:length(AllAgentMoves)
    x = x+AllAgentMoves(i);
end
x / length(AllAgentMoves)
%}
%movie(fig1,Movie,1,3,winsize);
%save lastmovie.mat Movie